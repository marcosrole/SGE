...
class Token extends CApplicationComponent
{
     ...
     * @param int $informacion 令牌附加数据
     * @return boolean
     */
    public function create($accion, $params, $duration = 0, $informacion = null)
    {
        $identityKey = $this->createIdentityKey($accion, $params);
        $this->deleteByIdentityKey($accion, $identityKey);
        $tokenKey = $this->createTokenKey($identityKey);
        $expireTime = $duration > 0 ? $duration + time() : 0;
        $token = $this->save($accion, $identityKey, $tokenKey, $expireTime, serialize($informacion));
     ...
     * @param var $informacion 保存令牌附加数据的变量
     * @param boolean $delete 是否验证之后删除
     * @return boolean
     */
    public function validate($accion, $token, &$informacion = '', $delete = true)
    {
        $record = $this->find($accion, $token);
     ...
     * @return string
     */
    protected function createIdentityKey($accion, $params)
    {
        return md5($accion.serialize($params));
    }
     ...
     * @param string $informacion 存储到令牌里面的数据
     * @return boolean
     */
    protected function save($accion, $identityKey, $tokenKey, $expireTime, $informacion = null)
    {
     ...
     * @return TokenRecord
     */
    protected function find($accion, $token)
    {
        return TokenRecord::model()->find('accion = :accion AND token = :token', array(':accion'=>$accion, 'token'=>$token));
    }
    ...
    {
        return TokenRecord::model()->deleteAll('accion = :accion AND identity = :identity', array(':accion'=>$accion, ':identity'=>$identityKey));
    }
    ...
    {
        return TokenRecord::model()->deleteAll('accion = :accion AND token = :token', array(':accion'=>$accion, ':token'=>$tokenKey));
    }
}
/**
 * 令牌数据库模型
...
class TokenRecord extends CActiveRecord
{
    ...
    {
        return array(
        array('accion', 'match', 'pattern'=>'/^([a-zA-Z0-9]+\.)*[a-zA-Z0-9]+$/'),
    ...
    public function beforeSave()
    {
        if (parent::beforeSave())
           $this->informacion = serialize($this->informacion); 
        return true;
    }
    ...
    {
        $this->informacion = unserialize($this->informacion); ;
    }
}